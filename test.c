...
int lpit0_ch0_flag_counter, lpit0_ch1_flag_counter, lpit0_ch2_flag_counter, lpit_ch3_flag_counter = 0;
...
unsigned int External_PIN=1;
uint32_t adcResultInMv3=0;
unsigned int flag,ht_flag = 0; /*flag for Water Motor,ht_sensor*/
...
void PORT_init (void){		
	...
	/* Interrupt input PORT C10~14*/
		...
		PTC->PDDR &= ~(1<<14);		/* Port C14 Pory Input set, value '0'*/
		...
		PORTC->PCR[14] |= PORT_PCR_MUX(1)|PORT_PCR_PE(1)|PORT_PCR_PS(1); // Port C14 mux = GPIO
		...
		PORTC->PCR[14] |=(10<<16); // Port C14 IRQC : interrupt on Falling-edge
...
void LPIT0_init (uint32_t delay){
  ...
  LPIT0->MCR |= LPIT_MCR_M_CEN_MASK;  /* DBG_EN-0: Timer chans stop in Debug mode */
	...
	LPIT0->TMR[3].TVAL = 40000;      /* Chan 3 Timeout period: 40000 = 1us */
  LPIT0->TMR[3].TCTRL = 0x00000001;
	
	LPIT0->TMR[2].TVAL = 40000000;      /* Chan 2 Timeout period: 40M = 1s */
  LPIT0->TMR[2].TCTRL = 0x00000001;
	
	LPIT0->TMR[1].TVAL = 60*40000000;      /* Chan 1 Timeout period: 60*40M = 60s */
...
void getHumidity(void) { //Humid&Temp Sensor
	...
	uint8_t recData[100];								/*8bit Data each (0,2 is each humid and temp)	*/
	int counter;
	
	PTD->PDDR |= (1<<10);	/*PORT D10 = output*/
	PTD->PDOR &= ~(1<<10);						/*PORT D10 = LOW	*/
	delay_ms(19);									/*Start Signal over 18ms	*/
	PTD->PDOR |= (1<<10);							/*PORT D10 = HIGH	*/
	...
	while(((PTD->PDIR)&(1<<10))==0){}		/*Wait for LOW	*/
		for(int i=0;i<40;i++){					/*40bit write start	*/
			while(((PTD->PDIR)&(1<<10))==1) {	/*PORT D10 = HIGH */
				start = 1;	/*start count*/
			}
				highDuty[i] = ht_flag;			/*highDuty[i] = HIGH time	*/
				start = 2;								/* clear flag	*/
		}
...
void PORTC_IRQHandler(void){
	...
	PORTC->PCR[14] &= ~(0x01000000); // Port Control Register ISF bit '0' set
	...
	if((PORTC->ISFR & (1<<10)) != 0){ //SW1 ON = Humid&Temp display by Bluetooth
		...
		if(adcResultInMv3<2000) {
			LPUART1_transmit_string("Enough Water\n");
		}
		else if(adcResultInMv3<4000)	{
	...
	PORTC->PCR[14] |= 0x01000000; // Port Control Register ISF bit '1' set
	}
...
//FTM0 Initialization
	FTM0->SC = FTM_SC_PWMEN0_MASK| FTM_SC_PWMEN1_MASK|FTM_SC_PWMEN3_MASK					//Enable PWM channel 1output
...
void FTM0_CH0_PWM (int i){//uint32_t i){ //FTM for Water Pump
	FTM0->CONTROLS[0].CnV=FTM_CnV_VAL(i);
...
void LPIT0_Ch0_IRQHandler (void){	  /* delay counter */
	lpit0_ch0_flag_counter++;         /* Increment LPIT1 timeout counter */
	LPIT0->MSR |= LPIT_MSR_TIF1_MASK;  /* Clear LPIT0 timer flag 0 */
}

void LPIT0_Ch1_IRQHandler (void){		/* delay = 60s counter */
	int adcResultInMv2 =0;
	lpit0_ch1_flag_counter++;         /* Increment LPIT0 timeout counter */
	covertAdcChan(14); /*Soil Humid sensor*/
	while(adc_complete()==0){} //Wait for conversion
	adcResultInMv2 = read_adc_chx();
	if(adcResultInMv2<2500) {
		FTM0_CH0_PWM(2500);
		flag = 1;
		convertAdcChan(12);
		while(adc_complete()==0){}            /* Wait for conversion complete flag 	*/
		adcResultInMv3 = read_adc_chx();
		if(adcResultInMv3>4000){
			PTD->PCOR |= (1<<11);
		}
	}
	LPIT0->MSR |= LPIT_MSR_TIF1_MASK;  /* Clear LPIT0 timer flag 0 */
...
void LPIT0_Ch3_IRQHandler (void){	/*flag counter for humid&temp sensor	*/
	lpit_ch3_flag_counter++;
	if(start == 1){
		ht_flag++;
	}
	else if(start == 2){
